...
let moduleRequest = (importUrl) => nativeFetch(importUrl, { mode: 'cors', credentials: 'include' }).then((res) => res.text());
...
function isBreaker(value) {
  try {
    ...
    return Array.isArray(value?.[EXECUTOR_BREAK_NAME]);
  } catch (err) {
    return false;
  }
}
